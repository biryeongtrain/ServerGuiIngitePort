 ...
 */
public enum ServerGuiClickType {
    ...
    ServerGuiClickType(boolean isLeft, boolean isRight, boolean isMiddle, boolean shift, int value, boolean numKey, boolean isDragging) {
        this.isLeft = isLeft;
    ...
    public static ServerGuiClickType toClickType(ClickType action, int button, int slot) {
        switch (action) {
            case PICKUP:
            ...
            case SWAP:
                if (button >= 0 && button < 9) {
                    return ServerGuiClickType.values()[button + 4];
                } else if (button == 40) {
                    return ServerGuiClickType.OFFHAND_SWAP;
                }
                break;
            case CLONE:
                return MOUSE_MIDDLE;
    ...
    public static ServerGuiClickType toClickType (org.bukkit.event.inventory.ClickType type, InventoryAction action, int button, int slot) {
        return switch (type) {
            case LEFT -> action == InventoryAction.HOTBAR_SWAP ? ServerGuiClickType.values()[button + 4] : MOUSE_LEFT;
            case RIGHT -> action == InventoryAction.HOTBAR_SWAP ? ServerGuiClickType.OFFHAND_SWAP : MOUSE_RIGHT;
            case SHIFT_LEFT -> MOUSE_LEFT_SHIFT;
            case SHIFT_RIGHT -> MOUSE_RIGHT_SHIFT;
            case MIDDLE -> MOUSE_MIDDLE;
            case DROP ->
                    slot == -999 ? (button == 0 ? MOUSE_LEFT_OUTSIDE : MOUSE_RIGHT_OUTSIDE) : (button == 0 ? DROP : CTRL_DROP);
            case CONTROL_DROP -> CTRL_DROP;
            case NUMBER_KEY -> ServerGuiClickType.values()[button + 4];
            case DOUBLE_CLICK -> MOUSE_DOUBLE_CLICK;
            case WINDOW_BORDER_LEFT -> MOUSE_LEFT_OUTSIDE;
            case WINDOW_BORDER_RIGHT -> MOUSE_RIGHT_OUTSIDE;
